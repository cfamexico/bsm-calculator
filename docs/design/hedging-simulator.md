...
Status: **proposal, no code yet**. This document is the whole PR. If the
direction is welcome, implementation follows in a second PR.
...
def delta_hedge_pnl(
    ...
    hedge_vol: Literal["implied", "real"] = "implied",
    option: Literal["call", "put"] = "call",
    position: Literal["short", "long"] = "short",
...
`tests/test_hedging_sim.py` (pytest; only new dev dependency). Fixed seeds,
tolerances sized to Monte Carlo error.
...
## 9. File changes (implementation PR)

```
hedging_sim.py                  new, ~150 lines
tests/test_hedging_sim.py       new
index.html                      one new <section>, one renderHedging() function, ~200 lines
README.md                       new subsection under "What It Does", Files tree
```

## 10. Open questions for the maintainer

1. Is a simulation section in scope for this project, or should the dashboard
   stay purely closed-form? If the latter, the Python module alone still stands.
2. Placement and title of the section.
3. Preference on tests: pytest file as proposed, or keep validation inside the
   `__main__` demo as the repo does today?
